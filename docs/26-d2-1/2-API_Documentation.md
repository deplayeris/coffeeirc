...
## 概述
CoffeeIRC是一个基于Java的轻量级IRC聊天库，采用HttpServer技术实现客户端-服务器通信架构。该库提供完整的聊天功能，包括连接管理、消息传递、广播系统、日志记录和内置加密通讯等功能。
...
##### 加密系统方法
```java
void initializeServerEncryption()
```
初始化服务器端加密系统，生成RSA密钥对和AES会话密钥

```java
String encryptAesKey(PublicKey clientPublicKey)
```
使用客户端公钥加密AES会话密钥

```java
String decryptClientMessage(String encryptedMessage, ClientInfo clientInfo)
```
解密来自客户端的加密消息
...
- `KeyPair serverRsaKeyPair`: 服务器RSA密钥对
- `SecretKey serverAesKey`: 服务器AES会话密钥
...
// 加密相关信息
PublicKey getClientPublicKey()         // 获取客户端公钥
void setClientPublicKey(PublicKey key) // 设置客户端公钥
SecretKey getAesKey()                  // 获取AES会话密钥
void setAesKey(SecretKey key)          // 设置AES会话密钥
boolean isEncryptionEnabled()          // 检查是否启用加密
void setEncryptionEnabled(boolean enabled) // 设置加密状态
```
...
```json
{
  ...
  "message": "连接成功",
  "clientId": "分配的客户端ID",
  "serverPublicKey": "服务器RSA公钥(Base64编码)",
  "encryptedAesKey": "加密的AES会话密钥(Base64编码)"
}
```

#### MessageHandler (消息处理器类)  
处理客户端消息发送请求的处理器类，支持加密消息解密
- **路由路径**: `/message`
- **请求方法**: POST
- **功能**: 接收并处理用户消息，支持特殊命令
...
```java
void decryptAesKey(String encryptedAesKeyStr)
```
解密服务器发送的AES会话密钥
...
- `KeyPair rsaKeyPair`: 客户端RSA密钥对
- `SecretKey aesKey`: AES会话密钥
- `PublicKey serverPublicKey`: 服务器公钥
- `boolean encryptionEnabled`: 加密是否启用标志
...
├── RSA KeyPair Generator (RSA密钥生成器)
└── AES Key Manager (AES密钥管理器)
...
├── RSA KeyPair Generator (RSA密钥生成器)
└── AES Encryptor/Decryptor (AES加解密器)
```

### 数据流向
1. **客户端连接**: Client → ConnectHandler → 加密握手 → ClientInfo注册
2. **消息发送**: Client → AES加密 → MessageHandler → 解密记录 → 聊天日志记录
3. **广播推送**: Server → AES加密 → PushHandler → 客户端解密接收
4. **断开连接**: Client → DisconnectHandler → 资源清理

---
*这个文档适用于 CoffeeIRC v26.d2.1 版本*